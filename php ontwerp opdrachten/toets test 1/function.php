...
function CrudBrouwer(){
 ...
 function PrintCrudBrouwer($result){
    ...
    foreach ($result as $row) {
        ...
        if (isset($row["brouwercode"])) {
            // Wijzig knopje
            $table .= "<td>". 
                "<form method='post' action='update_bier.php?brouwercode=$row[brouwercode]' >       
                        <button name='wzg'>Wzg</button>	 
                </form>" . "</td>";

            // Delete via linkje href
            $table .= '<td><a href="delete_bier.php?brouwercode='.$row["brouwercode"].'">verwijder</a></td>';
        }
...
function dropDown($label, $data){
    ...
    foreach($data as $row){
        $txt .= "<option value='$row[brouwercode]'>$row[naam]</option>";
    }
